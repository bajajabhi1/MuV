import java.io.File;

import com.amazonaws.services.s3.AmazonS3Client;
import com.amazonaws.services.s3.model.CannedAccessControlList;
import com.amazonaws.services.s3.model.PutObjectRequest;


public class S3BucketManager {
	AmazonS3Client s3;
	String 	bucket_name = null;
	
	public S3BucketManager(AmazonS3Client s3Instance, String bucketName)
	{
		this.s3 = s3Instance;
		this.bucket_name = bucketName;
		createBucket();
	}

	public void createBucket()
	{
		//create bucket
		s3.createBucket(bucket_name);
	}
	
	public void putOject(String key, File file)
	{		
		try {
			//put object - bucket, key, value(file)
			System.out.println("Putting object on S3");
			//PutObjectRequest(bucketName, objectKey, inputStream, metadata)
			s3.putObject(new PutObjectRequest(bucket_name, key, file).withCannedAcl(CannedAccessControlList.PublicRead));
			System.out.println("Done");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	/*public void getObject(String key)
	{
		try {
			//get object
			S3Object object = s3.getObject(new GetObjectRequest(bucket_name, key));
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(object.getObjectContent()));
			String data = null;
			while ((data = reader.readLine()) != null) {
				System.out.println("Retreived Date from S3 - " + data);
			}

		} catch (IOException e) {
			e.printStackTrace();
		}

	}*/
	
	public void deleteObject(String key)
	{
		s3.deleteObject(bucket_name, key);
		System.out.println("The object with key - " + key + " is deleted");
	}
	
	public void deleteBucket()
	{
		s3.deleteBucket(bucket_name);
		System.out.println("Deleted the bucket - " + bucket_name);
	}
}
