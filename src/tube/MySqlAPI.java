...
public class MySqlAPI {
	...
	private String selectAll = "select * from VIDEO_INFO order by rating desc";
	...
	public void deleteAll()
	{
		try {
			createConnectionAndStatement();
			String deleteSql = "DELETE FROM VIDEO_INFO";
			statement.executeUpdate(deleteSql);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private List<VideoVO> parseResultSet(ResultSet resultSet) throws SQLException {
		...
		while (resultSet.next()) {
			...
			Date date = new Date(resultSet.getTimestamp("timestamp").getTime() - 3600 * 5 * 1000);
